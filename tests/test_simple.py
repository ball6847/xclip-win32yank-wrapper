#!/usr/bin/env python3
"""
Simple test suite for xclip wrapper using pytest parameterization.
Converts simple_test.sh to Python with table-driven approach.
"""
...
# Simple test cases defined as a table
SIMPLE_TEST_CASES = [
    # (test_name, input_text, expected_text, check_type)
    ("Basic copy/paste", "Hello World", "Hello World", "exact"),
    ("Unicode", "Hello 世界 🎉", "Hello 世界 🎉", "exact"),
    ("Empty clipboard", "", "", "exact"),
    ("Special characters", "!@#$%^&*()_+-=[]{}|;:',.<>?/~`", "!@#$%^&*()_+-=[]{}|;:',.<>?/~`", "exact"),
]
...
@pytest.mark.parametrize("test_name,input_text,expected_text,check_type", SIMPLE_TEST_CASES)
def test_simple_operations(xclip_path, test_name, input_text, expected_text, check_type):
    """Test simple clipboard operations"""
    print(f"\nTest: {test_name}")
    ...
    try:
        ...
        clear_proc.communicate(input=b"")
...
def test_simple_error_handling(xclip_path, test_name, args, should_fail):
